document.addEventListener("DOMContentLoaded", function () {
    ...
    signUpButton.addEventListener("click", function (event) {
        ...
        if (!firstname || !lastname || !email || !password || !sbpw) {
            ...
            const emailRegex = /^[a-z0-9]+@+upi\.edu$/;
            const emailRegex1 = /^[a-z0-9]+@[a-z]+\.com$/;
            if (!emailRegex.test(email) && !emailRegex1.test(email)) {
                errorLabel2.textContent =
                    "Email input must be in the format @upi.edu or your format input is invalid";
                errorLabel2.style.visibility = "visible";
                isValid = false;
            }
            ...
            if (password !== sbpw) {
                errorLabel2.textContent =
                    "Input password and submit password do not match";
                errorLabel2.style.visibility = "visible";
                isValid = false;
